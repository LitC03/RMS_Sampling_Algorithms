% AGLORITHM A: uses greedy approach to sample an 2D Gaussian and
...
%% Get data of Gaussian

I = zeros(100);
I(50,50)=1000;
data = imgaussfilt(I,30);
name = 'GAUSS';

%% Set parameters
row_factor=4; % how many rows will be skipped in uniform sampling
col_factor=3; % how many columns will be skipped in uniform samling
...
% Perform thin plate cubic interpolation
...
%% Error calculation
% E1 = sqrt(sum((double(I)-NN_eq).^2,'all')./N)
% E2 = sqrt(sum((double(I)-NN_M).^2,'all')./N)
...
%% Save
% save(['algA_',name,'_F',num2str(row_factor),'X',num2str(col_factor),'_std_',num2str(sigma),'_sp',num2str(sp),'.mat'],'prob_map','M','seen','I','E1','E2','eq_samp','total_runs','NN_M','NN_eq','row_factor','col_factor','path_taken','sp','sigma')
